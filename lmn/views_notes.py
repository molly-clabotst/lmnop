...
def user_view_own_notes(request, user_pk):
    ...
    user_search_title = request.GET.get('user_search_title')
    if user_search_title:
        usernotes = Note.objects.filter(title__icontains=user_search_title).order_by('title')
    else:
